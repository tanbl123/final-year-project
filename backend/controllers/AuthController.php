...
function handleRegister(PDO $pdo): void {
  ...
  // stored exactly as the user entered it (no +60 prepend)
  ...
  try {
    ...
    $pdo->prepare(
      'INSERT INTO supplier
         (supplierId, userId, companyName, displayName, companyAddress,
       ...
       VALUES (:sid, :uid, :cn, :dn, :ca, :cl1, :cpc, :cc, :cst, :oa, :ol1, :opc, :oc, :ost, :brn, :blu, :tax)'
    )->execute([
      'sid' => $supplierId, 'uid' => $userId, 'cn' => $companyName, 'dn' => $companyName, 'ca' => $companyAddress,
...
function handleRegisterCustomer(PDO $pdo): void {
  ...
  // phone stored exactly as entered (no +60 prepend)
  $fmtErr = usernameFormatError($username);
  if ($fmtErr) {
    sendJson(400, false, null, ['code' => 'VALIDATION', 'message' => $fmtErr]);
  }
...
function validateCourierKyc(array $body): array {
  ...
  return [
    'fullName' => $fullName, 'phoneNumber' => $phoneNumber,   // stored as entered (no +60 prepend)
...
function handleMe(PDO $pdo, array $auth): void {
  ...
  $profile = null;
  if ($u['role'] === 'Supplier') {
    $p = $pdo->prepare('SELECT supplierId, companyName, displayName, displayNameUpdatedAt,
                               companyAddress, operationalAddress,
