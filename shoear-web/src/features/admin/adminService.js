...
// member to set their own password. Identity fields (phone, IC) are optional.
// Returns the created account (username, inviteEmailSent).
export function createStaff({ fullName, email, phoneNumber = '', icNumber = '', role = 'ArSpecialist' }) {
  return apiPost('/admin/staff', { fullName, email, phoneNumber, icNumber, role }, getToken());
}
