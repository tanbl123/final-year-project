...
// Platform-wide orders by status + overall on-time delivery.
function AdminOrderReport() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getAdminOrderReport({ from: range.from, to: range.to })
  ...
  function buildReportOpts() {
    return {
      title: 'Order & Fulfilment Report (Platform)',
      generatedBy: user?.fullName,
      period: range.label,
      referencePrefix: 'AOF',
      summary: [
        { label: 'Total orders', value: String(data.summary.totalOrders) },
        { label: 'Total order value', value: rm(data.summary.totalValue) },
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-end flex-wrap gap-2 mb-3">
        <p className="text-muted mb-0">All marketplace orders by status, and platform-wide on-time delivery.</p>
      ...
      {loading ? (
        ...
        <div className="card card-body text-center text-muted">No orders in this period.</div>
      ) : (
        <>
