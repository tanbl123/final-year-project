    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        when (call.method) {
            InputMethods.OPEN_SINGLE_LENS -> {
                ...
                // ShoeAR: use PLAY mode (live lens only) instead of CAPTURE mode.
                // Play has no photo/video capture button and saves no media, so the
                // try-on stays focused on the shoe and no storage/gallery permission
                // is ever needed.
                val intent = ARCameraActivity.Play.createIntent(
