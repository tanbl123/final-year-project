    ...
    companion object {
        ...
        fun intentFor(context: Context, configuration: Configuration): Intent {
            return Intent(context, ARCameraActivity::class.java).apply {
                ...
                // ShoeAR: open on the REAR camera (you point it at your feet), not
                // the selfie camera. Force facing=back and ignore the lens' own
                // facing preference; the flip button still lets the user switch.
                putExtra(EXTRA_CAMERA_FACING_FRONT, false)
                putExtra(EXTRA_CAMERA_FACING_FLIP_ENABLED, configuration.cameraFacingFlipEnabled)
                putExtra(EXTRA_CAMERA_FACING_BASED_ON_LENS, false)
